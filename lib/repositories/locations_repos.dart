...
// UPDATE FLAG
  static Future updateLocations(
      int id, String latitude, String longitude, String realAddress) async {
    try {
      final response = await http.put(
          Uri.parse('http://localhost:9999/pick-location/api/v1/get-loc/$id'),
...
// ADD LOCATIONS
 static Future addLocations(
      String latitude, String longitude, String address) async {
    try {
      final response = await http.post(
          Uri.parse('http://localhost:9999/pick-location/api/v1/get-loc'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
          },
          body: jsonEncode(<String, String>{
            'flag': '0', //to update flag to 1
            'latitude': latitude,
            'longitude': longitude,
            'address': address,
          }));
